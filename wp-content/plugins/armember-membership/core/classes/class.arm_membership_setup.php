...
if (!class_exists('ARM_membership_setup')) {
    ...
    class ARM_membership_setup {
       ...
       /* function arm_after_setup_form_submit($return_script){
            $return_script = "<script type='text/javascript' data-cfasync='false'>console.log('Repute log Test after setup'); </script>";
        ...
        function arm_setup_shortcode_func($atts, $content = "") {
            ...
            global $wp, $wpdb, $current_user, $ARMember, $arm_member_forms, $arm_global_settings, $arm_payment_gateways, $arm_subscription_plans, $bpopup_loaded, $ARMSPAMFILEURL;
            ...
            if ((!empty($args['id']) && $args['id'] != 0) || ($isPreview && !empty($args['setup_data']))) {
                ...
                if (!empty($setup_data) && !empty($setup_data['setup_modules']['modules'])) {
                    ...
                    if (!empty($current_user_plan_ids)) {
                        $current_user_plan = $current_user_plan_ids[0];
                    ...
                    $previuos_button_label = (isset($button_labels['previous']) && !empty($button_labels['previous'])) ? $button_labels['previous'] : __('Previous', 'ARMember');
                    $next_button_label = (isset($button_labels['next']) && !empty($button_labels['next']) )? $button_labels['next'] : __('Next', 'ARMember');
                                                        ...
                                                        if (is_user_logged_in()) {
                                                            if ($arm_user_old_plan == $plan_id) {
                                                                ...
                                                                if (empty($arm_user_payment_cycles)) {
                                                                    ...
                                                                    if ($completed_recurrence == $total_recurring || ($completed_recurrence == '' && $arm_user_selected_payment_mode == 'auto_debit_subscription')) {
                                ...
                                if (!empty($modules['gateways'])) {
                                    ...
                                    if (!empty($gateways)) {
                                        ...
                                        if (isset($setup_data['setup_labels']['payment_section_title']) && !empty($setup_data['setup_labels']['payment_section_title'])) {
                                            $module_content .= '<div class="arm_setup_section_title_wrapper" style="text-align:' . $formPosition . ';">' . $setup_data['setup_labels']['payment_section_title'] . '</div>';
                                        }
                                        ...
                                        $pglabels = isset($setup_data['arm_setup_labels']['payment_gateway_labels']) ? $setup_data['arm_setup_labels']['payment_gateway_labels'] : array();
                                            ...
                                            foreach ($gateways as $pg) {
                                                if (in_array($pg, array_keys($active_gateways))) {
                                                    ...
                                                    $pg_content .= '<div class="arm_module_gateway_name"><span class="arm_module_gateway_span">' . $pg_options['gateway_name'] . '</span></div>';
                                                    $pg_content .= '</label>';
                                                    switch ($pg) {
                                                        case 'paypal':
                                                            break;
                                                        ...
                                                        case 'bank_transfer':
                                                            ...
                                                            $pg_fields .= $arm_payment_gateways->arm_get_bank_transfer_form($pg_options, $fieldPosition, $errPosCCField);
                                                            $pg_fields .= '</div>';
                                                            $pg_fields .= '</div>';
                                                            break;
                                                        default:
                                        ...
                                        $arm_automatic_sub_label = (isset($setup_data['setup_labels']['automatic_subscription']) && !empty($setup_data['setup_labels']['automatic_subscription'])) ? $setup_data['setup_labels']['automatic_subscription'] : __('Auto Debit Payment', 'ARMember');
                                        $arm_semi_automatic_sub_label = (isset($setup_data['setup_labels']['semi_automatic_subscription']) && !empty($setup_data['setup_labels']['semi_automatic_subscription'])) ? $setup_data['setup_labels']['semi_automatic_subscription'] : __('Manual Payment', 'ARMember');
                                        ...
                                        $module_content .= "<div class='arm_setup_section_title_wrapper arm_payment_mode_selection_wrapper' >" . $setup_data['setup_labels']['payment_mode_selection'] . "</div>";
                                      ...
                                      global $current_user;
                                        ...
                                        if (!empty($current_user_plan_ids)) {
                                            ...
                                            foreach ($current_user_plan_ids as $plan_id) {
                                                ...
                                                $arm_user_selected_payment_mode = $planData['arm_payment_mode'];
                                                $arm_user_current_plan_detail = $planData['arm_current_plan_detail'];
                                                $plan_name_array[] = stripslashes($arm_user_current_plan_detail['arm_subscription_plan_name']);
                                                $plan_id_array[] = $plan_id;
                                    ...
                                    foreach ($plan_payment_cycles as $payment_cycle_plan_id => $plan_payment_cycle_data) {
                                        ...
                                        if (!empty($plan_payment_cycle_data)) {
                                            ...
                                            if (isset($setup_data['setup_labels']['payment_cycle_section_title']) && !empty($setup_data['setup_labels']['payment_cycle_section_title'])) {
                                                $module_content .= '<div class="arm_setup_section_title_wrapper arm_setup_payment_cycle_title_wrapper arm_hide" style="text-align:' . $formPosition . ';">' . $setup_data['setup_labels']['payment_cycle_section_title'] . '</div>';
                                            } else {
                                                ...
                                                if (in_array($pg, array_keys($active_gateways))) {
                                                    ...
                                                    switch ($pg) {
                                                        ...
                                                        case 'stripe':
                                                            ...
                                                            $pg_fields .= '<script data-cfasync="false">Stripe.setPublishableKey("' . $apikey . '");</script>';
        ...
        function arm_generate_setup_style($setupid = 0, $setup_style = array()) {
            ...
            $setup_css = '
                    ...
                    .arm_setup_form_' . $setupid . ' .arm_module_gateways_container .arm_module_gateway_fields{
                        font-family: ' . $setup_font_family . ', sans-serif, \'Trebuchet MS\';
                    }
                ';
            return $setup_css;
        }
        ...
        function arm_get_module_items_box($module_type = 'plans', $options = array()) {
            global $wp, $wpdb, $ARMember, $arm_slugs, $arm_global_settings, $arm_payment_gateways, $arm_subscription_plans;
            $module_box = '';
        ...
        function arm_save_membership_setups_func($posted_data = array()) {
            ...
            global $wp, $wpdb, $current_user, $arm_slugs, $ARMember, $arm_global_settings, $arm_payment_gateways;
        ...
        function arm_setup_shortcode_preview_func() {
            ...
            if (isset($_POST['action']) && $_POST['action'] == 'arm_setup_shortcode_preview' & isset($_POST['setup_data'])) {
                ...
                $setupData = esc_attr($_POST['setup_data']);
                $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_setups'], '1');
                ?>
