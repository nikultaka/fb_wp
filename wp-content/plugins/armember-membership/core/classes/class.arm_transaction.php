	...
	class ARM_transaction
	{
							...
							<div class="popup_content_text arm_transactions_detail_popup_text">
								<table width="100%" cellspacing="0">
									...
									<tr>
										...
										<td><?php echo $arm_payment_gateways->arm_amount_set_separator($log_detail['arm_currency'], $log_detail['arm_amount']) . ' ' . strtoupper($log_detail['arm_currency']);
		//echo (!empty($log_detail['arm_amount']) && $log_detail['arm_amount'] > 0) ? number_format((float) $log_detail['arm_amount'], 2) . ' ' . strtoupper($log_detail['arm_currency']) : '0.00';?></td>
									</tr>
									<tr>
		...
		function arm_add_manual_payment($data = array())
		{
			...
			if (!empty($data))
			{
				...
				$user_id = intval($manual_data['user_id']);
                ...
                function arm_get_user_transactions_with_pagging($user_id, $current_page = 1, $perPage = 2, $plan_id_name_array = array())
		{
			...
			$date_format = $arm_global_settings->arm_get_wp_date_time_format();
                        ...
                        if (!empty($user_id) && $user_id != 0) {
                                ...
                                $trans_records = '';
                                $trans_records .= '<div class="arm_user_transaction_wrapper" data-user_id="' . $user_id . '">';
				...
				$trans_records .= '<td>'.__('Membership','ARMember').'</td>';
					...
					$trans_records .= '<td>'.$transAmount.'</td>';
                                ...
                                if( $totalRecord <= 0 ){
                                    $trans_records .= '<tr>';
                                    $trans_records .= '<td colspan="9" style="text-align: center;">' . __('No Payment History Found.', 'ARMember') . '</td>';
                                    $trans_records .= '</tr>';                                    
                                }
                ...
                function arm_get_user_transactions_paging_action() {
                    ...
                    if(isset($_POST['action']) && $_POST['action'] == 'arm_get_user_transactions_paging_action') 
                    {
                        ...
                        echo $this->arm_get_user_transactions_with_pagging($user_id, $current_page, $per_page, $plan_id_name_array);
                    }
                    exit;
                }
		...
		function arm_get_bank_transfer_logs($filter_gateway = 0, $filter_ptype = 0, $filter_pstatus = 0, $limit = 0)
		{
			...
			if (empty($filter_gateway) || $filter_gateway == 'bank_transfer' || $filter_gateway == '0') {
				if (empty($filter_ptype) || $filter_ptype == 'one_time' || $filter_ptype == '0') {
                    ...
                    $logs = $wpdb->get_results("SELECT * FROM `".$ARMember->tbl_arm_payment_log."` $where_btlog ORDER BY `arm_created_date` DESC {$sqlLimit}");
					if (!empty($logs)) {
						foreach ($logs as $l) {
        ...
        function arm_load_transaction_grid() {
            ...
            global $wpdb, $ARMember, $arm_slugs, $arm_members_class, $arm_global_settings,  $arm_payment_gateways, $arm_subscription_plans, $armlite_default_user_details_text, $arm_capabilities_global;
            ...
            $total_count = $wpdb->get_results('SELECT COUNT(*) as total_logs FROM `' . $ARMember->tbl_arm_payment_log . '`');
            ...
            $total_payment_rows = "SELECT (SELECT COUNT(*) FROM `".$ARMember->tbl_arm_payment_log."` WHERE `arm_display_log` = 1) as total_payment_log";
            ...
            if (!empty($payment_log)) {
                ...
                foreach ($payment_log as $rc) {
                    ...
                    if ($rc->arm_payment_gateway == 'bank_transfer'):
                        $response_data[$ai][0] = '<input id="cb-item-action-' . $rc->arm_log_id . '" class="chkstanard arm_bt_transaction_bulk_check" type="checkbox" value="' . $rc->arm_log_id . '" name="item-action[]">';
                    else:
                        $response_data[$ai][0] = '<input id="cb-item-action-' . $rc->arm_log_id . '" class="chkstanard arm_transaction_bulk_check" type="checkbox" value="' . $rc->arm_log_id . '" name="item-action[]">';
                    endif;
                    ...
                    if (!isset($effectiveData[$rc->arm_user_id]) && !empty($change_plan) && $change_plan == $rc->arm_plan_id && $subscr_effective > strtotime($nowDate)) {
                        $response_data[$ai][5] .= '<div>' . __('Effective from', 'ARMember') . ' ' . date_i18n($date_format, $subscr_effective) . '</div>';
                    ...
                    if ($rc->arm_payment_gateway == 'bank_transfer' && $arm_transaction_status == 'pending') {
                        $gridAction .= "<a class='armhelptip arm_change_btlog_status' href='javascript:void(0)' onclick=\"ChangeStatus({$transactionID}, 1);armBpopup('change_transaction_status_message');\" data-status='1' data-log_id='" . $transactionID . "' title='" . __('Approve', 'ARMember') . "'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_approved.png' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_approved_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_approved.png';\" /></a>";
                        $gridAction .= "<a class='armhelptip arm_change_btlog_status' href='javascript:void(0)' onclick=\"ChangeStatus({$transactionID}, 2);armBpopup('change_transaction_status_message');\" data-status='2' data-log_id='" . $transactionID . "' title='" . __('Reject', 'ARMember') . "'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_denied.png' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_denied_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_denied.png';\" /></a>";
                    } 
                    
                    $gridAction .= "<a class='armhelptip arm_preview_log_detail' href='javascript:void(0)' data-log_type='" . $log_type . "' data-log_id='" . $transactionID . "' data-trxn_status='".$arm_transaction_status."' title='" . __('View Detail', 'ARMember') . "'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview.png' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview.png';\" /></a>";
                    $gridAction .= "<a href='javascript:void(0)' data-log_type='" . $log_type . "' data-delete_log_id='" . $transactionID . "' data-trxn_status='".$arm_transaction_status."' onclick='showConfirmBoxCallback({$transactionID});'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete.png' class='armhelptip' title='" . __('Delete', 'ARMember') . "' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete.png';\" /></a>";
                    $gridAction .= $arm_global_settings->arm_get_confirm_box($transactionID, __("Are you sure you want to delete this transaction?", 'ARMember'), 'arm_transaction_delete_btn', $log_type);
                    $gridAction .= "</div>";
            ...
            $columns = ',' . __('Transaction ID', 'ARMember') .',' . __('User', 'ARMember') . ',' . __('Membership', 'ARMember') . ',' . __('Gateway', 'ARMember') . ',' . __('Payment Type', 'ARMember') . ',' . __('Payer Email', 'ARMember') . ',' . __('Transaction Status', 'ARMember') . ',' . __('Payment Date', 'ARMember') . ',' . __('Amount', 'ARMember') . ',' . __('Credit Card Number', 'ARMember') . ',';
