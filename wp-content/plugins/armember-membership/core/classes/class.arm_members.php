...
if (!class_exists('ARM_members')) {
    ...
    class ARM_members {
        ...
        function __construct() {
            ...
            add_action('wp_ajax_arm_get_user_plan_failed_payment_details', array($this, 'arm_get_user_plan_failed_payment_details'));
        ...
        function arm_user_plan_action() {
            global $wpdb, $ARMember, $arm_member_forms, $arm_manage_communication, $arm_subscription_plans, $arm_members_class, $arm_global_settings, $arm_capabilities_global;
            $post_data = $_POST;
            ...
            if (isset($response['type']) && $response['type'] == 'success' && $user_ID > 0) {
                ...
                $response['membership_plan'] = '<span class="arm_user_plan_' . $user_ID . '">' . $plan_name . '</span>';
        ...
        function arm_get_user_all_plan_details($user_id = 0, $is_ajax = false) {
            ...
            if (!empty($user_id)) {
                ...
                $return .= '<div class="arm_add_new_item_box arm_add_new_plan"><a id="arm_change_plan_to_user" class="greensavebtn arm_save_btn" href="javascript:void(0)" ><img align="absmiddle" src="' . MEMBERSHIPLITE_IMAGES_URL . '/add_new_icon.png"><span> ' . __('Change Plan', 'ARMember') . '</span></a></div>';
                ...
                $return .= '<span class="arm_edit_plan_lbl">' . __('Select Plan', 'ARMember') . '*</span> ';
                ...
                $return .= '</dl>';
                $return .= '<br/><span class="arm_error_select_plan error arm_invalid" style="display:none; text-align:left;">' . __('Please select Plan.', 'ARMember') . '</span>';
                $return .= '</div>';
                $return .= '</div>';
                ...
                $return .= '<span class="arm_edit_plan_lbl">' . __('Plan Start Date', 'ARMember') . '</span>';
                ...
                $return .= '<div class="arm_edit_field">';
                $return .= '<button class="arm_member_add_plan_save_btn arm_save_btn">' . __('Save', 'ARMember') . '</button>';
                ...
                $return .= '<img src="' . MEMBERSHIPLITE_IMAGES_URL . '/arm_loader.gif" class="arm_loader_img_user_add_plan" style="position:relative;top:8px;display:none;" width="24" height="24" />';
                $return .= '</div>';
                $return .= '</div>';
                ...
                $return .= '<div class="arm_loading_grid arm_plan_loading_grid" style="display: none;"><img src="' . MEMBERSHIPLITE_IMAGES_URL . '/loader.gif" alt="Loading.."></div>';
                ...
                $return .= '<th class="arm_edit_plan_name">' . __('Membership Plan', 'ARMember') . '</th>';
                $return .= '<th class="arm_edit_plan_type">' . __('Plan Type', 'ARMember') . '</th>';
                        ...
                        if (!empty($planData)) {
                            ...
                            if (!empty($suspended_plan_ids)) {
                                if (in_array($uplans, $suspended_plan_ids)) {
                                    ...
                                    $arm_plan_is_suspended .= '<img src="' . MEMBERSHIPLITE_IMAGES_URL . '/grid_edit_hover_trns.png"  title="' . __('Activate Plan', 'ARMember') . '" class="armhelptip tipso_style" width="26" data-plan_id="' . $uplans . '" data-user_id="' . $user_id . '" onclick="showConfirmBoxCallback_plan(\'status_' . $uplans . '\');" style="margin: -5px 0; position: absolute; "/>';
                            ...
                            $arm_delete_plan .= '<img src="' . MEMBERSHIPLITE_IMAGES_URL . '/grid_delete_icon_trans.png"  title="' . __('Delete Plan', 'ARMember') . '" class="arm_edit_plan_action_button armhelptip tipso_style" id="arm_member_delete_plan" data-plan_id="' . $uplans . '" data-user_id="' . $user_id . '" onclick="showConfirmBoxCallback_plan(' . $uplans . ');"/>';
                            ...
                            $confirmBox .= "<button type='button' class='arm_confirm_box_btn armok arm_member_plan_delete_btn' data-item_id='{$uplans}'>" . __('Delete', 'ARMember') . "</button>";
                            ...
                            if ($expiry_date != '') {
                                ...
                                $arm_edit_plan .= "<a class='arm_member_edit_plan' >"
                                        . "<img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_edit_hover_trns.png' style='position: absolute; margin: -4px 0 0 5px; cursor: pointer;' width='26' title='" . __('Change Expiry Date', 'ARMember') . "' class='armhelptip tipso_style'/>"
                                        . "</a>";
                                $arm_edit_plan .= "<img src='" . MEMBERSHIPLITE_IMAGES_URL . "/arm_save_icon.png' style='vertical-align: middle;display:none;' width='14' height='16' title='" . __('Save Expiry Date', 'ARMember') . "' class='arm_edit_plan_action_button arm_member_save_plan armhelptip tipso_style' data-plan_id='" . $uplans . "' data-user_id='" . $user_id . "' />&nbsp;";
                                $arm_edit_plan .= "<img src='" . MEMBERSHIPLITE_IMAGES_URL . "/cancel_date_icon.png' style='display:none;' width='14' height='16' title='" . __('Cancel', 'ARMember') . "' class='arm_edit_plan_action_button arm_member_cancel_save_plan armhelptip tipso_style' data-plan_id='" . $uplans . "' data-user_id='" . $user_id . "' data-plan-expire-date='" . date('m/d/Y', $expiry_date) . "' />&nbsp;";
                                $arm_edit_plan .= '<img src="' . MEMBERSHIPLITE_IMAGES_URL . '/arm_loader.gif" class="arm_edit_user_plan_loader" style="    vertical-align: middle;display:none;margin-left: 10px;" width="17" height="18" />';
                            }
                            ...
                            $return .= '<td class="arm_edit_plan_name" >' . $plan_name . ' ' . $arm_plan_is_suspended . '</td>';
        ...
        function arm_manage_plan_get_cycle() {
            ...
            if (isset($_POST['action']) && !empty($_POST['action']) && isset($_POST['plan_id']) && !empty($_POST['plan_id'])) {
                ...
                if (!$plan->is_lifetime() && $plan->is_recurring()) {
                    ...
                    $plansCycleLists = '<li data-label="' . __('Select Payment Cycle', 'ARMember') . '" data-value="">' . __('Select Plan Cycle', 'ARMember') . '</li>';
                    ...
                    $arm_selected_payment_cycle = "arm_selected_payment_cycle";


                    $arm_plan_cycle_dropdown = '<input type="hidden" class="' . $arm_user_plan_cycle_data_id . '" name="' . $arm_selected_payment_cycle . '" id="' . $arm_user_plan_cycle_data_id . '" value=""/>';
                    ...
                    $arm_plan_cycle_dropdown .= '<br/><span class="arm_error_select_plan_cycle error arm_invalid" style="display:none; text-align:left;">' . __('Please select Plan cycle.', 'ARMember') . '</span>';


                    if (isset($_POST['arm_manage_plan_grid']) && $_POST['arm_manage_plan_grid'] == 1) {
                        $content .= '<span class="arm_edit_plan_lbl">' . __('Select Payment Cycle', 'ARMember') . '*</span> ';
        ...
        function arm_after_deleted_user_action($id, $reassign = 1) {
            ...
            $arm_members_detail = $wpdb->query("DELETE FROM `" . $ARMember->tbl_arm_members . "` WHERE `arm_user_id`='" . $id . "'");
            //Remove user's all Payment logs
            //$delete_payment_log = $wpdb->query("DELETE FROM `" . $ARMember->tbl_arm_payment_log . "` WHERE `arm_user_id`='" . $id . "'");
            //$delete_bt_log = $wpdb->query("DELETE FROM `" . $ARMember->tbl_arm_bank_transfer_log . "` WHERE `arm_user_id`='" . $id . "'");
            //Remove user's all activities
            //$delete_activity = $wpdb->query("DELETE FROM `" . $ARMember->tbl_arm_activity . "` WHERE `arm_user_id`='" . $id . "'");
        ...
        function arm_handle_import_user_meta() {
            ...
            if (isset($up_file)) {
                ...
                if (in_array($up_file_ext, array('csv', 'xls', 'xlsx', 'xml'))) {
                    if ($up_file_ext == 'xml') {
                        $fileContent = file_get_contents(MEMBERSHIPLITE_UPLOAD_DIR . '/' . basename($up_file));
        ...
        function arm_handle_import_user() {
            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_subscription_plans, $arm_case_types, $arm_member_forms;
            set_time_limit(0);
                ...
                if (in_array($up_file_ext, array('csv', 'xls', 'xlsx', 'xml'))) {
                    ...
                    if (!empty($users_array)) {
                        ...
                        function arm_add_import_user() {
                            ...
                            global $wpdb, $ARMember, $arm_members_class, $arm_global_settings, $arm_subscription_plans, $arm_case_types, $arm_member_forms, $arm_email_settings, $arm_capabilities_global;
                                                        ...
                                                        if (!empty($user_main_data['user_email'])) {
                                                            ...
                                                            if (is_wp_error($user_id)) {
                                                                ...
                                                                if ($send_notification == 'true') {
                                                                    ...
                                                                    // $subject = __('Welcome to ARMember', 'ARMember') . ' ' . get_option('blogname');
                                                                    // $message = '</td></tr></thead>';
                                                                    // $message .= '<tbody>';
                                                                    // $message .= '<tr>';
                                                                    // $message .= '<td>' . __('Username', 'ARMember') . ':</td><td> ' . $user_main_data['user_login'] . "</td>";
                                                                    // $message .= '</tr><tr>';
                                                                    // if (!empty($plaintext_pass)) {
                                                                    //     $message .= '<td>' . __('Password', 'ARMember') . ': </td><td>' . $plaintext_pass . "</td>";
                                                                    // }
                                                                    // $message .= '</tr></tbody>';
                                                                    // $message .= '</tabel>';
                        ...
                        function arm_user_import_handle($request) {
                            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_subscription_plans, $arm_case_types;
                            $file_data_array = $user_ids = $u_errors = $errors = array();
                        ...
                        function arm_user_export_handle($request) {
                            global $wp, $wpdb, $ARMember, $armPrimaryStatus, $arm_global_settings, $arm_subscription_plans, $arm_case_types;
                            $action = $request['arm_action'];
        ...
        function arm_change_user_status() {
            global $wpdb, $arm_email_settings, $ARMember, $arm_global_settings, $arm_subscription_plans, $arm_members_class, $arm_subscription_plans, $arm_manage_communication, $arm_slugs, $arm_payment_gateways, $arm_capabilities_global;
            ...
            if (!empty($user_id) && $user_id != 0) {
                ...
                if ((get_current_user_id() != $userID) && !is_super_admin($userID)) {
                    if ($primary_status == '3') {
                        ...
                        if (!empty($activation_key) && $activation_key != '') {
                            $gridAction .= "<a href='javascript:void(0)' onclick='showResendVerifyBoxCallback({$userID});'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/resend_mail_icon.png' class='armhelptip' title='" . __('Resend Verification Email', 'ARMember') . "' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/resend_mail_icon_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/resend_mail_icon.png';\" /></a>";
                ...
                $gridAction .= "<a class='arm_openpreview arm_openpreview_popup armhelptip' href='javascript:void(0)' data-id='" . $userID . "' title='" . __('View Detail', 'ARMember') . "'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview.png' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_preview.png';\" /></a>";
                if (current_user_can('arm_manage_members')) {
                    $edit_link = admin_url('admin.php?page=' . $arm_slugs->manage_members . '&action=edit_member&id=' . $userID);
                    $gridAction .= "<a href='" . $edit_link . "' class='armhelptip' title='" . __('Edit Member', 'ARMember') . "' ><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_edit.png' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_edit_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_edit.png';\" /></a>";
                }
                if ((get_current_user_id() != $userID) && !is_super_admin($userID)) {
                    $gridAction .= "<a href='javascript:void(0)' onclick='showChangeStatusBoxCallback({$userID});'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/change_status_icon.png' class='armhelptip' title='" . __('Change Status', 'ARMember') . "' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/change_status_icon_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/change_status_icon.png';\" /></a>";
                ...
                $gridAction .= "<a href='javascript:void(0)' onclick='arm_member_manage_plan({$userID});' id='arm_manage_plan_" . $userID . "'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_manage_plan_icon.png' class='armhelptip' title='" . __('Manage Plans', 'ARMember') . "' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_manage_plan_icon_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_manage_plan_icon.png';\" /></a>";
                ...
                if (current_user_can('arm_manage_members') && (get_current_user_id() != $userID)) {
                    if (is_multisite() && is_super_admin($userID)) {
                        ...
                        $gridAction .= "<a href='javascript:void(0)' onclick='showConfirmBoxCallback({$userID});'><img src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete.png' class='armhelptip' title='" . __('Delete', 'ARMember') . "' onmouseover=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete_hover.png';\" onmouseout=\"this.src='" . MEMBERSHIPLITE_IMAGES_URL . "/grid_delete.png';\" /></a>";
                ...
                $plan_name = (!empty($plan_names)) ? implode(',', $plan_names) : '-';
                $memberPlanText = '<span class="arm_user_plan_' . $userID . '">' . $plan_name . '</span>';
        ...
        function arm_get_member_type_text($user_id = 0) {
            ...
            global $wpdb, $ARMember, $arm_global_settings, $arm_subscription_plans, $arm_global_settings;
            $memberTypeText = '';
            $planIDs = get_user_meta($user_id, 'arm_user_plan_ids', true);
            $date_format = $arm_global_settings->arm_get_wp_date_format();
            if (!empty($planIDs) && is_array($planIDs)) {
        ...
        function arm_get_member_grid_data() {
            ...
            global $wpdb, $ARMember, $arm_slugs, $arm_members_class, $arm_member_forms, $arm_global_settings, $arm_subscription_plans, $arm_payment_gateways, $arm_capabilities_global;
            ...
            $grid_columns = array(
                ...
                'arm_primary_status' => __('Status', 'ARMember'),
                'roles' => __('User Role', 'ARMember'),
                'first_name' => __('First Name', 'ARMember'),
                'last_name' => __('Last Name', 'ARMember'),
                'display_name' => __('Display Name', 'ARMember'),
                'user_registered' => __('Joined Date', 'ARMember'),
            );
            ...
            $sel_administrator = "SELECT u.ID FROM `{$user_table}` u LEFT JOIN `{$usermeta_table}` um ON um.user_id = u.ID $user_where GROUP BY u.ID";
            ...
            $search_params = '';
            if ($sSearch != '') {
                $search_params = " AND ( u.user_login LIKE '%{$sSearch}%' OR u.user_email LIKE '%{$sSearch}%' OR u.display_name LIKE '%{$sSearch}%' OR (um.meta_key = 'first_name' AND um.meta_value LIKE '%{$sSearch}%') OR (um.meta_key = 'last_name' AND um.meta_value LIKE '%{$sSearch}%') OR (um.meta_key = '{$capability_column}' AND um.meta_value LIKE '%\"{$sSearch}\"%') )";
            }
            $search_where = "";
            ...
            //$search_query = "SELECT u.ID FROM `{$user_table}` u LEFT JOIN `{$usermeta_table}` um ON um.user_id = u.ID {$join_arm_user_table} {$search_where} GROUP BY u.ID";

            $search_query = "SELECT u.ID FROM `{$user_table}` u LEFT JOIN `{$usermeta_table}` um ON {$join_on} {$join_arm_user_table} {$search_where} GROUP BY u.ID";

            $tmp_user_query = $wpdb->get_results($search_query);
                    ...
                    foreach ($grid_columns as $key => $title) {
                        switch ($key) {
                            ...
                            case 'arm_user_plan_ids':
                                ...
                                $plan_name = (!empty($plan_names)) ? implode(',', $plan_names) : '';
                                $grid_data[$ai][$n] = '<span class="arm_user_plan_' . $userID . '">' . $plan_name . '</span>';
        ...
        function arm_manual_update_user_data($user_id = 0, $plan_id = 0, $posted_data = array(), $plan_cycle = 0) {
            ...
            global $arm_payment_gateways, $ARMember, $arm_members_class, $arm_subscription_plans;
            ...
            if ($plan->is_recurring()) {
                ...
                while ($total_occurence_actual > 0) {
                    ...
                    if ($start_time <= $current_time) {
                        ...
                        $return_array['arm_extra_vars'] = $return_array['arm_extra_vars'] = maybe_serialize(array('manual_by' => __('Paid By admin', 'ARMember')));
		...
		$plan_cycle_data_amount = str_replace(",", "", $plan->amount);
                ...
                $return_array['arm_response_text'] = '';
                $return_array['arm_extra_vars'] = maybe_serialize(array('manual_by' => __('Paid By admin', 'ARMember')));
                $return_array['arm_created_date'] = date('Y-m-d H:i:s', $start_time);
                $payment_log_id = $arm_payment_gateways->arm_save_payment_log($return_array);
            }
        }
