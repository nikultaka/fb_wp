...
class kode_extend_vc_addon_player_slider{
    function __construct() {
        // We safely integrate with VC with this hook
        //add_action( 'init', array( $this, 'kode_vc_notice' ) );
 
        // Use this when creating a shortcode addon
        add_shortcode( 'player_slider_item', array( $this, 'kode_print_element' ) );
        // Register CSS and JS
            ...
            "params" => array(
				...
				array(
					...
					'value' => array(
						...
						__( 'Style 1', 'kode-player' ) => 'style-1',
						__( 'Style 2', 'kode-player' ) => 'style-2',
					),
					'description' => __( 'Select Style.', 'kode-player' )
				),
				array(
					'type' => 'dropdown',
					'heading' => __( 'Select Column', 'kode-player' ),
					'param_name' => 'player_column_size',
					'value' => array(
						__( ' ', 'kode-player' ) => 'no-category',
						__( '2 Columns', 'kode-player' ) => '2',
						__( '3 Columns', 'kode-player' ) => '3',
						__( '4 Columns', 'kode-player' ) => '4',
						__( '6 Columns', 'kode-player' ) => '6',
					),
					'description' => __( 'Select Column.', 'kode-player' )
				),
				array(
					"type" => "textfield",
					"class" => "",
    ...
    */
    public function kode_print_element( $atts, $content = null ) {
		...
		$settings['style-layout'] = $style_layout;
		$settings['player-column-size'] = $player_column_size;
		$settings['num-title-fetch'] = $num_title_fetch;
		$settings['orderby'] = $orderby;		
		$settings['order'] = $order;
		$settings['pagination'] = $pagination;
		
		$output = '<div class="row">'.kode_get_player_item_slider($settings).'</div>';
		return $output;
    }
    /*
...
new kode_extend_vc_addon_player_slider();