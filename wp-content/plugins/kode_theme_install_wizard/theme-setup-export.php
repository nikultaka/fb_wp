...
add_action('init','export_dummy_data_for_wizard');
function export_dummy_data_for_wizard(){
	
	$default_content = array();
	// $post_types      = array( 'attachment', 'post', 'page' );
	$post_types      = array( 'attachment', 'wpcf7_contact_form', 'post', 'page', 'player', 'testimonial', 'team', 'product' );
	...
	foreach ( $post_types as $post_type ) {
		if ( in_array( $post_type, array( 'revision', 'event-recurring' ) ) ) {
			continue;
	...
	//			print_r($meta); exit;
			...
			foreach ( $meta as $meta_key => $meta_val ) {
				if (
					...
					(
						...
						strpos( $meta_key, 'post-option' ) === false &&
			...
			if ( $post_type == 'attachment' ) {
				...
				if ( is_file( $file ) ) {
					...
					if ( is_dir( ABSPATH . '/wp-content/plugins/kode_theme_install_wizard/images/stock/' ) ) {
						copy( $file, ABSPATH . '/wp-content/plugins/kode_theme_install_wizard/images/stock/' . basename( $file ) );
					}
				}
	...
	foreach ( $menus as $menu ) {
		if ( $menu->name == 'Menu 1' ) {
			$menu_ids['main_menu_left'] = $menu->term_id;
		} else if ( $menu->name == 'Menu 2' ) {
			$menu_ids['main_menu_right'] = $menu->term_id;
		}else if ( $menu->name == 'Main Navigation Responsive' ) {
	...
	$my_options = get_option('kickoff_admin_option');

	$dir = ABSPATH . '/wp-content/plugins/kode_theme_install_wizard/content/';
	if ( is_dir( $dir ) ) {
