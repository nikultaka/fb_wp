...
if (!class_exists('ARM_subscription_plans')) {
    ...
    class ARM_subscription_plans {
        ...
        function arm_save_subscription_plans_func($posted_data = array()) {
            global $wp, $wpdb, $arm_slugs, $ARMember, $arm_global_settings, $arm_access_rules, $arm_capabilities_global;
        ...
        function arm_get_comma_plan_names_by_ids($ids = array()) {
            ...
            if (!empty($ids)) {
                ...
                $plans = $wpdb->get_col("SELECT `arm_subscription_plan_name` FROM `" . $ARMember->tbl_arm_subscription_plans . "` WHERE `arm_subscription_plan_id` in ($plan_ids) ORDER BY `arm_subscription_plan_id` DESC");
        ...
        function arm_ajax_stop_user_subscription($user_id=0, $plan_id=0) {
            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_payment_gateways, $arm_members_class, $arm_capabilities_global;
        ...
        function arm_update_user_subscription($user_id = 0, $new_plan_id = 0, $action_by = '', $allow_trial = true, $arm_last_payment_status = 'success') {
            ...
            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_payment_gateways, $arm_manage_communication,$arm_members_class, $arm_subscription_plans;
        ...
        function arm_update_user_subscription_for_bank_transfer($user_id = 0, $new_plan_id = 0, $payment_gateway = 'bank_transfer', $payment_cycle = 0, $arm_last_payment_status = 'success') {
            ...
            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_payment_gateways, $arm_manage_communication,  $arm_members_class;
            if (!empty($user_id) && $user_id != 0) {
                arm_set_member_status($user_id, 1);
                /* Only update plan if Member is active */
        ...
        function arm_membership_history_paging_action() {
            ...
            if (isset($_POST['action']) && $_POST['action'] == 'arm_membership_history_paging_action') {
                ...
                echo $this->arm_get_user_membership_history($user_id, $current_page, $per_page, $plan_id_name_array);
            }
            exit;
        }
        ...
        function arm_get_user_membership_history($user_id = 0, $current_page = 1, $perPage = 2, $plan_id_name_array = array()) {
            ...
            if (!empty($user_id) && $user_id != 0) {
                ...
                $totalRecord = $wpdb->get_var("SELECT COUNT(`arm_activity_id`) FROM `" . $ARMember->tbl_arm_activity . "` WHERE `arm_type`='membership' AND `arm_user_id`='$user_id'");
                $historyRecords = $wpdb->get_results("SELECT * FROM `" . $ARMember->tbl_arm_activity . "` WHERE `arm_type`='membership' AND `arm_user_id`='$user_id' AND `arm_action` != 'recurring_subscription' ORDER BY `arm_activity_id` DESC {$historyLimit}", ARRAY_A);
                if (!empty($historyRecords)) {
                    ...
                    $historyHtml .= '<tr>';
                    $historyHtml .= '<td>' . __('Plan', 'ARMember') . '</td>';
