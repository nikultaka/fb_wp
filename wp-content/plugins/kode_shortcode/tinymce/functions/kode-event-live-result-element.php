...
class kode_extend_vc_addon_event_live_result {
    function __construct() {
        // We safely integrate with VC with this hook
        //add_action( 'init', array( $this, 'kode_vc_notice' ) );
 
        // Use this when creating a shortcode addon
        add_shortcode( 'event_live_result_item', array( $this, 'kode_print_element' ) );
        // Register CSS and JS
        ...
        vc_map(array(
            ...
            "params" => array(
				array(
					...
					"value" => __(" ", 'vc_extend'),
					"description" => __("This is header title which will manage header title.", 'vc_extend')
				), 
				array(
					'type' => 'checkbox',
				...
				array(
					'type' => 'dropdown',
					'heading' => __( 'Show Fixtures Tab', 'js_composer' ),
					'param_name' => 'fixtures_show',
					'value' => array(
						__( ' ', 'js_composer' ) => 'no-category',
						__( 'Current Fixtures', 'js_composer' ) => 'current',
						__( 'Fixture Results', 'js_composer' ) => 'result',
						__( 'Upcomming Fixtures', 'js_composer' ) => 'future',
					),
    ...
    */
    public function kode_print_element( $atts, $content = null ) {
		...
		$settings['fixtures-show'] = $fixtures_show;
		$settings['margin-bottom'] = $margin_bottom;
		$output = '<div class="row">'.kode_get_live_result_item($settings).'</div>';
		return $output;
    }
    /*
...
new kode_extend_vc_addon_event_live_result();