<?php
/*
Plugin Name: KodeForest Theme Installation Wizard
Plugin URI: 
Description: KodeForest Core Theme Installation Wizard
Version: 1.0
Author: Adeel Nazar
Author URI: http://www.kodeforest.com
License: 
Text-Domain: wp-installation-wizard
*/
/**
...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function get_logo_image() {
			...
			if ( $logo_image_id ) {
				...
				$image_url = esc_url(get_template_directory_uri() . '/images/logo.png');
			}
		...
		public function init_globals() {
			...
			$this->envato_username = apply_filters( $this->theme_name . '_theme_setup_wizard_username', 'kodeforest' );
			$this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', 'http://kodeforest.com/envato/ap/server-script.php' );
		...
		public function setup_wizard() {
			...
			wp_register_script( 'jquery-blockui', plugins_url( '/js/jquery.blockUI.js', __FILE__ ), array( 'jquery' ), '2.70', true );
			wp_register_script( 'envato-setup', plugins_url( '/js/envato-setup.js', __FILE__ ), array(
				'jquery',
				'jquery-blockui',
			), $this->version );
			...
			wp_enqueue_style( 'envato-setup', plugins_url( '/css/envato-setup.css', __FILE__ ), array(
				'wp-admin',
				'dashicons',
				'install',
			), $this->version );
	...
	public function setup_wizard_header() {
		...
		<body class="envato-setup wp-core-ui kodeforest-body">
		<h1 id="wc-logo">
			<a href="http://themeforest.net/user/kodeforest/portfolio" target="_blank"><?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				@include( 'theme-setup-export.php' );
		...
		public function envato_setup_default_content() {
			...
			// $ddd = json_encode($custom_options);
			// update_option( 'kickoff_admin_option', $ddd );
			// print_r($ddd);
			?>
			<form method="post">
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			$local_file = trailingslashit(  ABSPATH ) . 'wp-content/plugins/kode_theme_install_wizard/images/stock/' . $file_name;
			$upload     = false;
		...
		public function _content_install_settings() {
			...
			$custom_options = $this->_get_json( 'options.json' );			
			update_option( 'kickoff_admin_option', $custom_options );
			...
			$blogpage = get_page_by_title( 'Blogs' );
			if ( $blogpage ) {
	                    ...
	                    foreach ( $this->site_styles as $style_name => $style_data ) {
                            ...
                            <li <?php echo $style_name == $current_style ? ' class="current" ' : ''; ?>>
                                <a href="#" data-style="<?php echo esc_attr( $style_name ); ?>"><img
                                            src="<?php echo esc_url(WPJB_URL .'/setup_wizard/images/'.$style_name.'/style.jpg');?>"></a>
                            </li>
	                    <?php } ?>
                    </ul>
                </div>
		...
		public function envato_setup_customize() {
			...
			<p>
				Most changes to the website can be made through the Appearance > Theme Options from the WordPress
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="http://kodeforest.com/support" target="_blank">http://kodeforest.com/support</a>
				and includes:</p>
			<ul>
			...
			<ul>
				<li>Customization services (this is available through <a
						href="http://kodeforest.com/support"
						target="_blank">KodeForest Studio</a>)
				</li>
		...
		public function envato_setup_ready() {
			...
			<a href="https://twitter.com/share" class="twitter-share-button"
			   data-url="http://themeforest.net/user/kodeforest/portfolio?ref=kodeforest"
			...
			<p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. <br/>Follow <a href="https://twitter.com/kodeforest" target="_blank">@KodeForest</a>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					...
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="https://twitter.com/kodeforest"
						                             target="_blank"><?php esc_html_e( 'Follow @KodeForest on Twitter' ); ?></a>
						</li>
				...
				<div class="envato-setup-next-steps-last">
					...
					<ul>
						<li class="documentation"><a href="https://wordpress.org/support/"
						...
						<li class="support"><a href="http://kodeforest.com/support/"
...
// include_once('theme-setup-export.php');