<?php
global $wpdb, $armPrimaryStatus, $ARMember, $arm_slugs, $arm_members_class, $arm_member_forms, $arm_global_settings, $arm_subscription_plans, $arm_social_feature,$arm_email_settings;
/**
        ...
        <div class="arm_add_edit_member_wrapper arm_member_detail_box">
                                    ...
                                    if (in_array($meta_key, $arm_member_include_fields_keys) && !in_array($meta_key,array('section', 'roles', 'html', 'hidden', 'submit','repeat_email','social_fields'))) {
                                        ...
                                        <?php if($meta_key=='user_pass'){
                                            ...
                                            <tr class="form-field">
                                                ...
                                                ?>
                                                    <input id="arm_password" autocomplete="off" class="arm_member_form_input <?php echo $arm_suffix_icon_pass_cls; ?>" name="user_pass" type="password" value="" data-msg-required="<?php _e('Password can not be left blank.', 'ARMember'); ?>" <?php if ($required_class != 1): ?>required<?php endif; ?>/>
            ...
            <div class="arm_admin_form_content">
                <table class="form-table">
                        <tr class="form-field">
                            <th>
                                <label for="arm_role"><?php _e('Role', 'ARMember'); ?></label>
                            </th>
                                        ...
                                        <div class="arm_confirm_box_body">
                                            ...
                                            <div class="arm_confirm_box_text arm_text_align_left arm_padding_top_15">
                                                ...
                                                <div style="display: inline-block; position: relative;" class="arm_plan_start_date_box arm_margin_top_10">
                                                    ...
                                                    <input type="text" value="<?php echo $plan_start_date; ?>"  name="arm_subscription_start_date" class="arm_member_form_input arm_user_plan_date_picker arm_width_232 arm_min_width_232"/>
                                                </div>
                                            </div>
                                                        ...
                                                        if (!empty($planData)) {
                                                            ...
                                                            if (!empty($suspended_plan_ids)) {
                                                                if (in_array($pID, $suspended_plan_ids)) {
                                                                    $arm_plan_is_suspended = '<div class="arm_user_plan_status_div arm_position_relative" ><span class="armhelptip tipso_style arm_color_red" id="arm_user_suspend_plan_' . $pID . '" style=" cursor:pointer;" onclick="arm_show_failed_payment_history(' . $user_id . ',' . $pID . ',\'' . $planName . '\',\'' . $planData['arm_start_plan'] . '\')" title="' . __('Click here to Show failed payment history', 'ARMember') . '">(' . __('Suspended', 'ARMember') . ')</span><img src="' . MEMBERSHIPLITE_IMAGES_URL . '/grid_edit_hover_trns.png" width="26" style="position: absolute; margin: -4px 0 0 5px; cursor: pointer;" title="' . __('Activate Plan', 'ARMember') . '" data-plan_id="' . $pID . '" onclick="showConfirmBoxCallback(\'change_user_plan_' . $pID . '\');" class="arm_change_user_plan_img_' . $pID . '">
...
<span id="arm_user_expiry_date_' . $pID . '" style="display: inline;">' . date_i18n($date_format, $planData['arm_expire_plan']) . ' <img src="' . MEMBERSHIPLITE_IMAGES_URL . '/grid_edit_hover_trns.png" width="26" style="position: absolute; margin: -4px 0 0 5px; cursor: pointer;" title="' . __('Change Expiry Date', 'ARMember') . '" data-plan_id="' . $pID . '" class="arm_edit_user_expiry_date"></span><span id="arm_user_expiry_date_box_' . $pID . '" style="display: none; position: relative; width: 155px;"><input type="text" value="' . date('m/d/Y', $planData['arm_expire_plan']) . '"  name="arm_subscription_expiry_date_' . $pID . '" class="arm_member_form_input arm_user_plan_expiry_date_picker" style="width: 120px; min-width: 120px;"/><img src="' . MEMBERSHIPLITE_IMAGES_URL . '/cancel_date_icon.png" width="11" height="11" title="' . __('Cancel', 'ARMember') . '" data-plan_id="' . $pID . '" data-plan-expire-date="' . date('m/d/Y', $planData['arm_expire_plan']) . '" class="arm_cancel_edit_user_expiry_date"></span>


' : __('Never Expires', 'ARMember');
                                                        $renewal_on = !empty($planData['arm_next_due_payment']) ? date_i18n($date_format, $planData['arm_next_due_payment']) : '-';
                                                        $trial_starts = !empty($planData['arm_trial_start']) ? $planData['arm_trial_start'] : '';
                                                        $trial_ends = !empty($planData['arm_trial_end']) ? $planData['arm_trial_end'] : '';
                                                        $arm_payment_mode = ( $planData['arm_payment_mode'] == 'auto_debit_subscription') ? '<br/>(' . __('Auto Debit','ARMember') . ')' : '';
                                                        ...
                                                        <tr class="arm_user_plan_table_tr <?php echo ($count_plans % 2 == 0) ? 'even' : 'odd'; ?>" id="arm_user_future_plan_div_<?php echo $count_plans; ?>">
                                                            ...
                                                            <td>
                                                                <input type="hidden" name="arm_user_future_plan[]" value="<?php echo $pID; ?>" id="arm_user_future_plan_<?php echo $pID; ?>"/>
...
<div class="arm_member_plan_failed_payment_popup popup_wrapper" >
    ...
    <div class="popup_content_text arm_member_plan_failed_payment_popup_text arm_text_align_center" >
        ...
        <div class="arm_width_100_pct" style=" margin: 45px auto;"> <img src="<?php echo MEMBERSHIPLITE_IMAGES_URL . "/arm_loader.gif"; ?>"></div>
...
<script>
    ...
    var SELECTPLANLABEL = '<?php _e('Select Plan', 'ARMember'); ?>';
    var PLANSTARTDATELABEL = '<?php echo __('Plan Start Date', 'ARMember').' '; ?>';
    var CURRENTDATE = '<?php echo date("m/d/Y"); ?>';
    var REMOVEPLAN = '<?php _e('Remove Plan', 'ARMember'); ?>';
    var ADDPLAN = '<?php _e('Add New Plan', 'ARMember'); ?>';
    var REMOVEPLANMESSAGE = '<?php _e('You cannot remove all plans.', 'ARMember'); ?>';
    var IMAGEURL = "<?php echo MEMBERSHIPLITE_IMAGES_URL; ?>";
    var ACTIVESTATUSLABEL = "<?php _e('Active', 'ARMember'); ?>";
</script>