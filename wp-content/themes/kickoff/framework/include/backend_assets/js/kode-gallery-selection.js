(function($){
	...
	$.fn.kodeCreateGallerySelection = function(){
		...
		add_button.click(function(){
			kodeSelectSliderImage(textarea);
		});
		...
		container.sortable({
			...
			stop: function(event, ui){
				kodeUpdateSliderOrder(textarea);
			}
		});
		...
		kodeCreateSliderSlide(textarea);
	}
	
	// update the slider order when takes an action
	function kodeUpdateSliderOrder(textarea){
	...
	function kodeCreateSliderSlide(textarea){
		var slider_data = (textarea.val().length > 0)? $.parseJSON(textarea.val()): [];
		...
		for (var i=0; i<slide_order.length; i++){ 
			...
			slide.click(function(){
				kodeCreateSliderOption(textarea, $(this).attr('data-id'));
			});
	...
	function kodeSelectSliderImage(textarea){
		var slider_data = (textarea.val().length > 0)? $.parseJSON(textarea.val()): [];
		var slide_order = (slider_data[0])? slider_data[0]: [];
		var slides = (slider_data[1])? slider_data[1]: {};
	...
	function kodeCreateSliderOption(textarea, image_id){
		...
		editbox_overlay.click(function(){
			kodeCloseSliderOption(editbox, textarea, image_id, overlay);
		});
		...
		var slider_option = kodeInitSliderOption(textarea, image_id, caption);
	...
	function kodeInitSliderOption(textarea, image_id, caption){
		...
		slider_option.append(
			'<div data-name="gallery_title" data-title="Title" data-type="text" data-wrapper-class="" ></div>\
			<div data-name="gallery_caption" data-title="Caption" data-type="text" data-wrapper-class="" ></div>\
	...
	function kodeCloseSliderOption(editbox, textarea, image_id, overlay){
		var slider_data = $.parseJSON(textarea.val());
