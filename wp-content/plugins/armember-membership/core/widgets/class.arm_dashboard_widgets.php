	...
	class armAdminDashboardWidgets
	{
		...
		function armAdminDashboardWidgets_init()
		{
			if ( current_user_can('administrator') || current_user_can('arm_admin_dashboard_widgets') )
			{
				...
				wp_add_dashboard_widget('armember-add-ons', esc_html__('ARMember Pro Add-Ons', 'ARMember'), array($this, 'armember_dashboard_widgets_add_ons_list'));
		...
		function ARMUserTransactions_display()
		{
			...
			$date_format = $arm_global_settings->arm_get_wp_date_format();
			$pay_log = $wpdb->get_results("SELECT * FROM `".$ARMember->tbl_arm_payment_log."` ORDER BY `arm_created_date` DESC", ARRAY_A);
		...
		function ARMemberSummary_display()
		{
			...
			<div class="arm_plugin_logo">
				<img alt="" src="<?php echo MEMBERSHIPLITE_IMAGES_URL;?>/arm_logo.png"/>
			</div>
