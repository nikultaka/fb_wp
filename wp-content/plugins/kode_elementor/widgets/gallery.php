...
class Main_Gallery extends Widget_Base {
	...
	public function get_title() {
		return __( 'KodeForest Gallery', 'elementor-main-gallery' );
	}
		  	...
		  	[
		   	'label'       	=> esc_html__( 'Type', 'essential-addons-elementor' ),
		     	...
		     	'options' 		=> [				
					...
					'style-3'=> esc_html__('Style 3', 'baldiyaat')
		     	],
		  	]
		);
	...
	protected function render() {		
		...
		?>
		<div class="main-gallery-wrapper kode-gallery kode-gutter-gallery bottom-spacer kode-style-gal-<?php echo esc_attr($settings['style']);?> main-gallery-<?php echo esc_attr( $this->get_id() ); ?>">
			<?php 
			...
			if($settings['style'] == 'style-1'){
				...
				foreach($settings['slider'] as $slide_id => $slides){
					if( ($current_size >= ($paged - 1) * $settings['num-fetch']) &&
						...
						$image_id = $slides['main_gallery_settings_slide']['id'];
						$image_src = wp_get_attachment_image_src($image_id, $settings['thumbnail-size']);	
						if(!empty($image_src)){
							echo '
							<li class="gallery-item columns ' . wpha_get_column_class_updated('1/' . $settings['gallery-columns']) . '">
								<figure class="kode-ux">
									<img src="'.esc_url($image_src[0]).'" alt="gallery title">
									<figcaption><a rel="prettyphoto[gallery]" data-gal="prettyphoto[gallery-'.$current_size.']" class="kode-gallery-hover thbg-color" href="'.esc_url($image_src[0]).'"><i class="fa fa-plus"></i></a></figcaption>
								</figure>
							</li>'; // gallery column				
						}else{
							echo '
							<li class="gallery-item columns ' . wpha_get_column_class_updated('1/' . $settings['gallery-columns']) . '">
								<figure class="kode-ux">
									<img src="'.esc_url($slides['main_gallery_settings_slide']['url']).'" alt="gallery title">
									<figcaption><a rel="prettyphoto[gallery]" data-gal="prettyphoto[gallery-'.$current_size.']" class="kode-gallery-hover thbg-color" href="'.esc_url($slides['main_gallery_settings_slide']['url']).'"><i class="fa fa-plus"></i></a></figcaption>
								</figure>
							</li>'; // gallery column				
						}			
					}
			...
			if( $settings['pagination'] == true ){
				echo kode_get_pagination($num_page, $paged);
			}
