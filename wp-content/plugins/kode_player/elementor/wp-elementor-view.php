...
*/
	...
	function wpha_players_filter_category($settings){
		$players = '';
		...
		if( empty($settings['category']) ){
			...
			$term = get_term_by('slug', $settings['category'], 'player-category');
		...
		if(isset($settings['filter-category']) && $settings['filter-category'] == 'show'){
			...
			$players_category = $parent + explode(',', $settings['category']);
			$players .= '<div class="top-category-list"><ul data-type="print_all_playerss" data-style="'.esc_attr($settings['style']).'" class="kf_filtrable_pinter" data-nouce="'.esc_attr(wp_create_nonce('validate-post-information')).'" data-ajax="'.esc_url(admin_url( 'admin-ajax.php' )).'">';
			foreach($players_category as $filter_id => $filter){	
				$term = get_term_by('term_id', $filter, 'player-category');			
				$players .= '<li class="' . esc_attr($filter_active) . '">';
				$players .= '<span class="kode-button" ';
				$players .= 'data-value="' . esc_attr($filter) . '" >' . esc_attr($filter) . '</span></li>';
				$filter_active = '';
			}
			$players .= '</ul></div>';
		...
		$players_ajax  = '<div class="wpha-loader-wrapper"><span></span></div>';
		
		return array('html'=>$players,'jax_wrapper'=>$players_ajax);
	}
	
	function wpha_filter_query_exe_player($settings,$args){
		//Get Query
	...
	function wpha_filter_query_html_player($query,$settings){
		$players = '';
									...
									<div class="row">
										<div class="col-md-6">
											<select onchange="this.form.submit()" name="orderby" class="chosen-select">
												<option value="any">'.esc_attr__('Any','kode-players-list').'</option>';
												$players  .= '
												<option ';
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'date'){$players  .= 'selected';}
												$players  .= '
												 value="date">'.esc_attr__('Publish Date','kode-players-list').'</option>';
												$players  .= '
												<option ';
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'title'){$players  .= 'selected';}
												$players  .= ' value="title">'.esc_attr__('Title','kode-players-list').'</option> <option ';							
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'rand'){$players  .= 'selected';}
												$players  .= ' 
												 value="rand">'.esc_attr__('Random','kode-players-list').'</option>
											</select>
										</div>
										<div class="col-md-6">
											<select onchange="this.form.submit()" name="order" class="chosen-select">
												...
												$players  .= '
												<option ';
												if(isset($_GET['order']) && $_GET['order'] == 'asc'){$players  .= 'selected';}
												$players  .= '
												 value="asc">'.esc_attr__('ASC','kode-woo-list').'</option>';
												$players  .= '
												<option ';
												if(isset($_GET['order']) && $_GET['orderby'] == 'desc'){$players  .= 'selected';}
												$players  .= ' value="desc">'.esc_attr__('DESC','kode-woo-list').'</option>
											</select>
										</div>
									</div>	
								</div>
	...
	function wpha_get_players_item( $settings = array() ){
		...
		if( !empty($settings['category']) || !empty($settings['tag']) ){
			...
			if( !empty($settings['category']) ){
				array_push($args['tax_query'], array('terms'=>explode(',', $settings['category']), 'taxonomy'=>'player-category', 'field'=>'slug'));
			}
		...
		$filter = wpha_players_filter_category($settings);
		...
		//Execution of Filter
		$args = wpha_filter_query_exe_player($settings,$args);
		
		$query = new WP_Query( $args );
		
		$ret .= wpha_filter_query_html_player($query,$settings);
		...
		// get players by the players style
		...
		if($settings['style'] == 'style-1'){
			$ret .= '<div class="wpha-listing-item kode-players kode-players-classic '.esc_attr($settings['padding']).'">';
			$ret .= '<div class="row">';
			$wpha_post_settings['thumbnail_size'] = $settings['thumbnail_size'];
			$wpha_post_settings['num_excerpt'] = intval($settings['num_excerpt']);
			$wpha_post_settings['title_num_fetch'] = $settings['title_num_fetch'];
			$players_size = $settings['column_size'];	
			$ret .= wpha_get_players_full($query, $players_size, $settings);
			$ret .= '</div>';
			$ret .= '</div>';
			...
			$ret .= '<div class="wpha-listing-item kode-players kode-players-classic '.esc_attr($settings['padding']).'">';
			$ret .= '<div class="row">';
			$ret .= wpha_get_players_grid($query, $players_size, $settings);			
			$ret .= '</div>';
			$ret .= '</div>';
			...
			$wpha_post_settings['num_excerpt'] = intval($settings['num_excerpt']);
			$wpha_post_settings['title_num_fetch'] = $settings['title_num_fetch'];
			$ret .= '<div class="wpha-listing-item kode-players kode-players-classic">';
			$ret .= '<div class="row">';
			$ret .= wpha_get_players_list($query, $players_size, $settings);
			$ret .= '</div>';
			$ret .= '</div>';
			...
			$ret .= '<div class="wpha-listing-item kode-players kode-players-classic '.esc_attr($settings['padding']).'">';
			$ret .= '<div class="row">';
			$ret .= wpha_get_players_widget($query, $players_size, $settings);
			$ret .= '</div>';
			$ret .= '</div>';
			...
			$ret .= '<div class="wpha-listing-item kode-players kode-players-classic '.esc_attr($settings['padding']).'">';
			$ret .= '<div class="row">';
			$ret .= wpha_get_players_modern_normal_5($query, $players_size, $settings);
			$ret .= '</div>';
			$ret .= '</div>';
		}else{
			$wpha_post_settings['thumbnail_size'] = $settings['thumbnail_size'];
	...
	//////////////////////////////////////////////////////////////////
	function wpha_get_players_info( $players_id='', $array = array(), $wrapper = true, $sep = '',$div_wrap = 'div' ){
		...
		foreach($array as $post_info){
			...
			switch( $post_info ){
				case 'date':
					...
					break;
				case 'features':
					$tag = get_the_term_list($players_id, 'features', '', ' ' , '' );
					if(empty($tag)) break;					
					...
					break;
				case 'state':
					$category = get_the_term_list($players_id, 'state', '', ' ' , '' );
				...
				case 'comment':
					...
					$ret .= '<a href="' . esc_url(get_permalink($players_id)) . '#respond" >' . esc_attr(get_comments_number()) . ' Comments</a>';						
					$ret .= '</'.esc_attr($div_wrap).'>';					
					break;
				case 'views':
					$ret .= '<'.esc_attr($div_wrap).' class="wpha-info wpha-views"><i class="fa fa-eye"></i>';
					$ret .= '<a href="' . esc_url(get_permalink($players_id)) . '" >' . esc_attr(wpha_get_players_views(get_the_ID())) . '<em>' . esc_html__('Views','real-estate').' </em></a>';
					$ret .= '</'.esc_attr($div_wrap).'>';							
					break;		
				case 'author':
					ob_start();
		...
		if($wrapper && !empty($ret)){
			return '<div class="wpha-playersadvisor-info wpha-info-new">' . $ret . '<div class="clear"></div></div>';
		}else if( !empty($ret) ){
			return $ret;
		}
		return '';			
	...
	function wpha_get_players_list($query, $size, $settings){
		
		if(isset($settings['listing']) && $settings['listing'] == 'slider'){ return wpha_get_players_grid_carousel($query, $size, $settings); }
		...
		global $post;
			...
			$ret .= '<div class="kode-ux kode-players-widget-ux">';
			ob_start();
			...
			$item_id = 'vt_players_'.$post->ID;
			// $votes = wpha_get_voting_by_id($item_id);
			
			// $settings['votes'] = $votes;
			
			$ret .= wpha_get_players_style_3( $settings );
	...
	function wpha_get_players_widget($query, $size, $settings){
		...
		global $post;
			...
			$ret .= wpha_get_players_style_4( $settings );
	...
	function wpha_get_players_grid($query, $size, $settings){
		
		if(isset($settings['listing']) && $settings['listing'] == 'slider'){ return wpha_get_players_grid_carousel($query, $size, $settings); }
		
		
		$ret = ''; $current_size = 0;			
		//$ret .= '<div class="kode-isotope" data-type="players" data-layout="' . $players_layout  . '" >';
		while($query->have_posts()){ $query->the_post();
		global $post;
			if( $current_size % $size == 0 ){
			...
			$ret .= '<div class="kode-ux kode-players-grid-ux">';
			ob_start();
			$wpha_post_option = wpha_decode_stopbackslashes(get_post_meta(get_the_ID(), 'post-option', true ));
			if( !empty($wpha_post_option) ){
				$wpha_post_option = json_decode( $wpha_post_option, true );
			...
			$ret .= wpha_get_players_style_2( $settings );
	...
	function wpha_get_players_full($query, $size, $settings){
		...
		global $post;
			...
			$ret .= '<div class="kode-item kode-players-single-full ">';
			ob_start();
			$wpha_post_option = wpha_decode_stopbackslashes(get_post_meta(get_the_ID(), 'post-option', true ));
			if( !empty($wpha_post_option) ){
				$wpha_post_option = json_decode( $wpha_post_option, true );
			...
			$ret .= wpha_get_players_style_1( $settings );
	...
	function wpha_get_players_modern_normal($query, $size, $settings){
	...
	function wpha_get_players_modern_normal_5($query, $size, $settings){
		...
		global $post;
			...
			$ret .= wpha_get_players_style_5( $settings );
	...
	function wpha_get_players_grid_carousel($query, $size,$settings){
	...
	function wpha_get_players_style_2( $settings ){
		$mypost = $settings['post'];
		$image_src_full = $settings['image_src_full'];
		$title_num_fetch = $settings['title_num_fetch'];
		
		$players = '
		<div class="city_senior_player">
			<figure class="box">
			...
			<div class="city_senior_player_text">
	...
	function wpha_get_players_style_3( $settings ){
		$mypost = $settings['post'];
		$all_review = '';
		$image_src_full = $settings['image_src_full'];
		$title_num_fetch = $settings['title_num_fetch'];
		...
		return '		
			<div class="players-grid-03 hover-effect-01">
				<figure>
					'.get_the_post_thumbnail($mypost->ID, $settings['thumbnail_size']).'
	...
	function wpha_get_players_style_5( $settings ){
		$mypost = $settings['post'];
		$image_src_full = $settings['image_src_full'];
		$title_num_fetch = $settings['title_num_fetch'];
	...
	function wpha_related_player_member($post_id) {
		global $post,$council_admin_option;	
		
		$tags = wp_get_post_terms($post_id, 'player-category', array("fields" => "all"));
		$tag_arr = '';
		if($tags) {
			if(isset($tags)){
				foreach( $tags as $tag ) {
				...
				if( !empty($tag_arr)){
					...
					if( !empty($tag_arr)){
						array_push($args['tax_query'], array('terms'=>explode(',', $tag_arr), 'taxonomy'=>'player-category', 'field'=>'slug'));
					}				
				}
