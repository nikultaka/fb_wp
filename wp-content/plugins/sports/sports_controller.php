            ...
            if (!empty($notExistUsers)) {
                ...
                foreach ($notExistUsers as $key => $value) {
                    ...
                    if (!empty($oldUserData)) {
                        ...
                        $sql = "select * from " . $autojointeamtable . " where leagueid = " . $hdnleagueid . " and userid=" . $value;
                        $joinedData = $wpdb->get_results($sql);
                        $joinedTeamData = array();
                        foreach ($joinedData as $joinedDatakey => $joinedDataValue) {
                            $joinedTeamData[] = $joinedDataValue->teamnameid;
                        }

                        $teamDiff = array_diff($teamID, $joinedTeamData);

                        if (!empty($teamDiff)) {
                            $teamDiff = array_values($teamDiff);
                        } else {
                            // $sql =  "DELETE FROM " . $autojointeamtable . " WHERE userid = " . $userid ;
                            $wpdb->delete($autojointeamtable, array('userid' => $value));                            
                            $teamDiff = $teamID;
                        }
                        ...
                        if ($containsAllValues == 1) {
                            ...
                            if (!empty($matchAutoData)) {
                                ...
                                if ($matchAutoData[0]->team1 == $teamAutoSelectedID) {
                                    ...
                                    $autojoinMatchAuto = array('userid' => $value, 'leagueid' => $leagueID, 'roundid' => $updateId, 'matchid' => $autoMatchID, 'teamid' => 0, 'teamnameid' => $teamAutoSelectedID);
                                }
